...
function setupClaudeConfig() {
  try {
    console.log(
      "🔧 Setting up Claude configuration for prediction-markets-mcp...",
    );
    ...
    console.log(
      "✅ Successfully configured Claude to use prediction-markets-mcp",
    );
    console.log(`📄 Config file: ${CLAUDE_CONFIG_FILE}`);
    console.log("");
    console.log(
      "🚀 You can now use prediction markets in Claude by asking questions like:",
    );
    console.log(
      '   • "What are the current odds for Trump winning the election?"',
    );
    console.log('   • "Show me prediction markets about the Supreme Court"');
    console.log('   • "What are the latest odds on PredictIt?"');
    console.log("");
    console.log(
      "💡 Note: You may need to restart Claude Desktop for the changes to take effect.",
    );
  } catch (error) {
